...
:: We need these XX__ temporary variables to deal with quote-less comparisons

(set CFLAGS_CONDA_NVCC_BACKUP__=%CFLAGS_CONDA_NVCC_BACKUP:"=%)
if not "%CFLAGS_CONDA_NVCC_BACKUP__%"=="" (
    (set CFLAGS=%CFLAGS_CONDA_NVCC_BACKUP%)
    set "CFLAGS_CONDA_NVCC_BACKUP="
)
set "CFLAGS_CONDA_NVCC_BACKUP__="

(set CPPFLAGS_CONDA_NVCC_BACKUP__=%CPPFLAGS_CONDA_NVCC_BACKUP:"=%)
if not "%CPPFLAGS_CONDA_NVCC_BACKUP__%"=="" (
    (set CPPFLAGS=%CPPFLAGS_CONDA_NVCC_BACKUP%)
    set "CPPFLAGS_CONDA_NVCC_BACKUP="
)
set "CPPFLAGS_CONDA_NVCC_BACKUP__="

(set CXXFLAGS_CONDA_NVCC_BACKUP__=%CXXFLAGS_CONDA_NVCC_BACKUP:"=%)
if not "%CXXFLAGS_CONDA_NVCC_BACKUP__%"=="" (
    (set CXXFLAGS=%CXXFLAGS_CONDA_NVCC_BACKUP%)
    set "CXXFLAGS_CONDA_NVCC_BACKUP="
)
set "CXXFLAGS_CONDA_NVCC_BACKUP__="
